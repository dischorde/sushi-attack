...
import { randomBetween, playBlip, playBloop } from "./util.js";
...
class Game {
  ...
  displayInMiddle(text, color) {
    let style = "40px Vampiro One";
    let popup = this.popUpText(text, style, color, 300);
    popup.y += 20;
  }
  ...
  askToPlay() {
    let popup = new createjs.Text("Play Again?", "180px Vampiro One", "#c7f330");
    popup.x = this.centerX;
    popup.y = this.centerY - (popup.getBounds().height * 2.25) / 2;
    popup.textBaseline = "top";
    popup.lineWidth = .9 * this.canvas.width;
